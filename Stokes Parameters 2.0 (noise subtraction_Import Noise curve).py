...
#UPDATE: This code add abs(S1), abs(S2), abs(S3) calculation
...
for root, dirs, files in os.walk(yourpath, topdown=False):
    ...
    for name in files[:-1]:                                 # we only want 0 -> 350
        ...
        for line in md:
            line = line.split()
            intensity_lst.append(int(float(line[1])) - noise_lst[noise_index])      #subtract noise value
...
#ABSOL S1
S1_abs_lst = [abs(i) for i in S1_lst]                           #ABSOL(S1), ColL: Absolute value of col C

#ABSOL S2
S2_abs_lst = [abs(i) for i in S2_lst]                           #ABSOL(S2), ColM: Absolute value of col D

#ABSOL S3
S3_abs_lst = [abs(i) for i in S3_lst]                           #ABSOL(S3), ColN: Absolute value of col E

#ABSOLS1/S0
S1_S0_abs_lst = [abs(i) for i in S1_S0_lst]                     #ABSOL(S1/S0), ColO: Absolute value of col G

#ABSOLS2/S0
S2_S0_abs_lst = [abs(i) for i in S2_S0_lst]                     #ABSOL(S2/S0), ColP: Absolute value of col H

#ABSOLS3/S0
S3_S0_abs_lst = [abs(i) for i in S3_S0_lst]                     #ABSOL(S3/S0), ColQ: Absolute value of col I

orient_lst = [0.5*rad_to_deg(math.atan(s2i/s1i)) for s2i, s1i in zip(S2_lst, S1_lst)]                                       #ColR: Orientation angle (degrees)

ellip_lst = [0.5*rad_to_deg(math.atan(s3i/math.sqrt(s1i**2 + s2i**2))) for s3i, s1i, s2i in zip(S3_lst, S1_lst, S2_lst)]    #ColS: Ellipticity (degrees)



#5 Create a new file containing the stokes parameters:
new_file = open("Stokes parameters (no correction) (noise subtracted _ Import Noise Curve).txt", "w")
new_file.write('Wavelength(nm)' + ' ' + 'S0' + ' ' + 'S1' + ' ' + 'S2' + ' ' + 'S3' 
                + ' ' + 'PolDeg' + ' ' + 'S1/S0' + ' ' + 'S2/S0' + ' ' + 'S3/S0' 
                + ' ' + 'S0-Min' + ' ' + 'S0-Min/Max' + ' ' + 'ABSOL(S1)'
                + ' ' + 'ABSOL(S2)' + ' ' + 'ABSOL(S3)' + ' ' + 'ABSOL(S1/S0)'
...
for i in range(len(S0_lst)):
    new_file.write(str(wavelength_lst[i]) + ' ' + str(S0_lst[i]) + ' ' + str(S1_lst[i]) + ' ' + str(S2_lst[i]) + ' ' + str(S3_lst[i])
                   ...
                   + ' ' + str(S0_min_lst[i]) + ' ' + str(S0_minmax_lst[i]) + ' ' + str(S1_abs_lst[i])
